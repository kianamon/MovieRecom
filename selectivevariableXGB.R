...
names(df_movies)[names(df_movies) == 'title_year'] <- 'Year'
#####################################################################################
...
movies5 <- df_movies %>%
  select(Title, URL, Score, Duration, Genres, Directors, Num_Votes, Year) 
...
movies1$Duration[is.na(movies1$Duration)] <- round(mean(movies1$Duration, na.rm = TRUE))
movies1$Year[is.na(movies1$Year)] <- round(mean(movies1$Year, na.rm = TRUE))
colSums(is.na(movies1))

kiana <- df_kianamovies %>%
  select(Title, URL, Score, Duration, Genres, Directors, Num_Votes, Year, KianaRating)
...
df_kianatrain$Likability[is.na(df_kianatrain$Likability)] <- round(mean(df_kianatrain$Likability, na.rm = TRUE))
df_kianatrain$Duration[is.na(df_kianatrain$Duration)] <- round(mean(df_kianatrain$Duration, na.rm = TRUE))
colSums(is.na(df_kianatrain))
head(df_kianatrain)
df_kianatrain1 <- df_kianatrain %>%
  select(-Title, -URL, -Directors)
moviesfinal <- movies1 %>%
  select(-Title, -URL, -Directors)

write.csv(df_kianatrain, file = "df_kiana.csv", row.names=F)
write.csv(movies1, file = "df_movies.csv", row.names=F)
#####################################################################################
#
...
smp_size <- floor(0.80 * nrow(df_kianatrain1))
# set the seed to make your partition reproducible
set.seed(123)
train_ind <- sample(seq_len(nrow(df_kianatrain1)), size = smp_size)
train <- df_kianatrain1[train_ind, ]
test <- df_kianatrain1[-train_ind, ]
head(train)
#####################################################################################
########################################XGBoost######################################
#####################################################################################
...
newrealtest <- model.matrix(~.+0,data = moviesfinal)
...
finalpredicted <- 10*floor(finalpred1/10)
colSums(is.na(movies1))
#####################################################################################
#submission
recom2 <- data.frame(Title=movies1$Title, URL=movies1$URL, Score=movies1$Score,
                     ...
                     Num_Votes=movies1$Num_Votes, Year=movies1$Year, 
                     Likability=finalpredicted,
